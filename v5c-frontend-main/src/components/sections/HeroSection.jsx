  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className="text-[12px] md:text-[24px] text-white tracking-[0.2em] uppercase">
            {content.categories.split('|').map((cat, i, arr) => (
              <React.Fragment key={i}>
                <span className={cat.trim().toUpperCase() === 'APARTMENTS' ? 'font-normal' : 'font-black'}>
                  {cat}
                </span>
                {i < arr.length - 1 && ' | '}
              </React.Fragment>
