...
const Navbar = ({ lang, setLang, content, user, onLogout, transparentNav = false }) => {
  ...
  const getNavRoute = (navItem, index) => {
    const routes = ['/about', '/properties', '/services', '/developers', '/why-dubai', '/faq', '/contact'];
    return routes[index] || '/';
  };
  ...
  const handleLanguageSwitch = (newLang) => {
    setLang(newLang);
    // Explicitly preserve current route
  ...
  useEffect(() => {
    document.body.style.overflow = (menuOpen || isPageLoading) ? 'hidden' : 'unset';
  }, [menuOpen, isPageLoading]);
        ...
        <div className="w-full max-w-[1500px] mx-auto px-6 lg:px-12 flex items-center justify-between">
          ...
          <div className="hidden xl:flex items-center gap-5.5 text-[11px] font-bold tracking-[0.15em] uppercase ml-auto mr-16 transition-colors duration-500 text-black font-sans">
            {(content?.nav || []).map((item, index) => {
              const route = getNavRoute(item, index);
              if (route === '/contact') {
                return (
                  <motion.div key={item} whileHover={{ y: -2 }}>
                    <button
                      onClick={handleContactClick}
                      className="relative group transition-colors py-2 hover:text-black/60 uppercase"
                    >
                      {item}
          ...
          >
            ...
            <div className="flex flex-wrap gap-3 justify-center relative z-10">
              {['EN', 'RU', 'ES'].map(l => (
                <span
                  key={l}
                  onClick={() => { handleLanguageSwitch(l); setMenuOpen(false); }}
